...
export function TagManagementModal({ isOpen, onClose, tags, onUpdateTags }: TagManagementModalProps) {
  ...
  const handleDelete = (tag: string) => {
    ...
    toast.error(`Tag "${tag}" excluída.`);
  };

  const handleEdit = () => {
    if (!editingTag || !editingTag.new || tags.includes(editingTag.new)) return;
    const newTags = tags.map(t => t === editingTag.old ? editingTag.new : t);
        ...
        >
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="space-y-2 max-h-[300px] overflow-auto pr-2">
              {tags.map(tag => (
                <div key={tag} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 group">
                  {editingTag?.old === tag ? (
                    ...
                    <>
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => setEditingTag({old: tag, new: tag})} className="p-1 text-gray-300 hover:text-blue-500"><Edit2 size={14} /></button>
                        <button onClick={() => handleDelete(tag)} className="p-1 text-gray-300 hover:text-red-500"><Trash2 size={14} /></button>
                      </div>
                    </>
                  )}
                </div>
